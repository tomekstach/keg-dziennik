...
require_once($CFG->dirroot . '/local/addteachers/classes/form/filter.php');
...
$PAGE->requires->js_call_amd('local_addteachers/config', 'init', array(get_string('nrdziennika', 'local_addteachers'), get_string('nrdziennikafull', 'local_addteachers'), get_string('edit', 'local_addteachers')));

$templatecontext = (object) [
  'texttodisplay' => get_string('localusertext', 'local_addteachers'),
...
foreach ($students as $student) {
  ...
  if ($roles[$role]->shortname == 'student') {
    if ($student->lastaccess > 0) {
      ...
      $student->lastaccess = get_string('never', 'local_addteachers');
    }
